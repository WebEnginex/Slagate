...
export const preloadImagesInCache = async (
...
): Promise<void> => {  // Si le tableau est vide, ne rien faire
  if (!urls || urls.length === 0) return;
  
  devLog(`🔄 Préchargement de ${urls.length} images...`);
  ...
  worker.onmessage = (e) => {
    ...
    if (type === "getImage" || !type) {
      ...
      if (completed % 10 === 0 || completed === total) {
        devLog(`📦 Préchargement: ${completed}/${total} images (${Math.round(completed/total*100)}%)`);
      }
      ...
      if (completed === total) {
        devLog(`✅ Préchargement terminé: ${completed} images dans le cache`);
        worker.terminate();
      }
    }
  };
  ...
  worker.onerror = (error) => {
    devError("❌ Erreur dans le worker de préchargement:", error);
