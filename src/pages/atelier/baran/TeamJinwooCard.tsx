...
import { TeamBaranJinwoo } from "@/config/atelier/baran/teamBaranJinwoo";
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
                  ...
                  <div className="bg-sidebar/50 rounded-lg overflow-hidden">
                    ...
                    >
                      <CollapsibleTrigger className="w-full p-4 font-medium flex items-center gap-1.5 text-sm text-white border-b border-sidebar-border">
                        ...
                        <span className="flex-1 text-left">Statistiques à viser</span>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <div className="p-4">
              ...
              >
                <div className="flex flex-col items-center">
                  <p className="mb-2 text-xs font-semibold text-solo-light-purple">
                    {slot.charAt(0).toUpperCase() + slot.slice(1)} {/* Capitalisation */}
                  </p>
                  <img
                    src={art?.image || ""}
      ...
      <div className="p-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[team.competence1, team.competence2].map((id, i) => {
            ...
            if (!skill) return null; // Afficher uniquement si "skill" existe
            return (
              <div
                key={i}
                            ...
                            <div className="p-4">
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {[team.qte1, team.qte2].map((id, i) => {
                                  ...
                                  if (!qte) return null; // Afficher uniquement si "qte" existe
                                  return (
                                    <div
                                      key={i}
                                      className="bg-sidebar p-4 rounded-lg border border-sidebar-border flex flex-col sm:flex-row items-center sm:items-start gap-4"
    ...
    <CollapsibleContent>
      <div className="p-4">
        <div className="space-y-4">
          <div>
            <h6 className="text-xs font-medium mb-3 text-solo-light-purple ml-1">
              Pierres Booster
            </h6>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[team.pierre_benediction_booster1, team.pierre_benediction_booster2, team.pierre_benediction_booster3, team.pierre_benediction_booster4].map((id, i) => {
                const pierre = getFromList(pierres, id);
                if (!pierre) return null;
                return (
                  <div
                    key={i}
                    className="bg-sidebar p-3 rounded-lg border border-sidebar-border flex flex-col items-center"
                  >
                    <img
                      src={pierre.image || ""}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[team.pierre_benediction_survie1, team.pierre_benediction_survie2, team.pierre_benediction_survie3, team.pierre_benediction_survie4].map((id, i) => {
                const pierre = getFromList(pierres, id);
                if (!pierre) return null;
                return (
                  <div
