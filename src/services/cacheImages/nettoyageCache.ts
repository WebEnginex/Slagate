...
export const resetAllCaches = async (): Promise<boolean> => {
  try {
    devLog("🧹 Nettoyage complet des caches...");
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        deleteRequest.onsuccess = () => {
          devLog("✅ Base IndexedDB supprimée avec succès");
          resolve();
        };
        
        deleteRequest.onerror = () => {
          devError("❌ Erreur lors de la suppression de la base IndexedDB");
          reject(new Error("Échec de la suppression de la base IndexedDB"));
        };

        deleteRequest.onblocked = () => {
          devWarn("⚠️ Suppression de la base IndexedDB bloquée - fermeture des connexions");
      ...
      devError("❌ Échec de la réinitialisation du cache IndexedDB", error);
      return false;
    }
  } catch (error) {
    devError("❌ Erreur globale lors de la réinitialisation des caches", error);
    return false;
  }
};
...
export const syncCacheStatus = async (): Promise<boolean> => {
  ...
  if (currentFlag && actualStatus) {
    localStorage.setItem("indexedDBFailed", "false");
    devLog("🔄 Flag indexedDBFailed corrigé: false");
    return true;
  } 
  else if (!currentFlag && !actualStatus) {
    localStorage.setItem("indexedDBFailed", "true");
    devLog("🔄 Flag indexedDBFailed corrigé: true");
    return true;
  }
...
export const displayCacheStats = async () => {
  // Si nous ne sommes pas en mode développement, ne rien faire
